...
the rename replaces it. It writes nothing else there and removes nothing it did not
...
value too large to put on a clock: above `2**63 - 1` the arithmetic that schedules an
interval cannot convert it, and it is refused at both doors rather than left to surface
as an `OverflowError` one tick later. `--ignore-snapshot` takes no value of its own:
passed, the file at `--snapshot-path` is never read, whether or not it is readable, so a
good snapshot is discarded as readily as a corrupt one. The server starts with an empty
...
nothing ever overwrites it. It is the escape hatch for a file that refuses to load. Left
set permanently -- in a unit file, say -- it starts every restart with an empty
keyspace, not just the first, because it never reads the file.
