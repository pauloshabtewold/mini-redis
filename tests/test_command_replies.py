...
def test_reply_bytes(conn, argv, expected):
    assert commands.dispatch(conn, argv) == expected


@pytest.mark.parametrize("bad", [b"1", b"3", b"9", b"0", b"-1", b"abc", b"2.0", b"", b"02"])
def test_hello_refuses_every_protocol_version_but_two(conn, bad):
    assert commands.dispatch(conn, [b"HELLO", bad]) == b"-NOPROTO unsupported protocol version\r\n"


@pytest.mark.parametrize("argv", [[b"HELLO"], [b"HELLO", b"2"]])
def test_hello_answers_the_fourteen_element_array(conn, argv):
    expected = (
        b"*14\r\n$6\r\nserver\r\n$5\r\nredis\r\n$7\r\nversion\r\n$5\r\n0.1.0\r\n"
        b"$5\r\nproto\r\n:2\r\n$2\r\nid\r\n:%d\r\n$4\r\nmode\r\n$10\r\nstandalone\r\n"
        b"$4\r\nrole\r\n$6\r\nmaster\r\n$7\r\nmodules\r\n*0\r\n" % conn.id
...
def test_hello_reports_the_replication_role_not_the_connection_role():
    ...
    try:
        assert b"$4\r\nrole\r\n$6\r\nmaster\r\n" in commands.dispatch(follower, [b"HELLO"])
...
def test_hello_reports_this_connection_s_own_id():
    ...
    try:
        assert commands.dispatch(first, [b"HELLO"]) != commands.dispatch(second, [b"HELLO"])
        assert b"$2\r\nid\r\n:%d\r\n" % first.id in commands.dispatch(first, [b"HELLO"])
    finally:
        a.close()
        c.close()
...
def test_an_unknown_command_is_one_frame_carrying_the_name_as_sent(conn, name, expected):
    reply = commands.dispatch(conn, [name])
    assert reply == expected
    assert reply.count(b"\r\n") == 1


def test_an_unknown_command_ignores_its_arguments(conn):
    # the short form is pinned; real Redis appends the first few arguments and this never does
    assert commands.dispatch(conn, [b"NOSUCHCMD", b"foo", b"bar"]) == b"-ERR unknown command 'NOSUCHCMD'\r\n"
...
def test_arity_is_exact_when_positive_and_a_minimum_when_negative():
    ...
    def xarity(conn, argv):
        return b"+XOK\r\n"

    try:
        assert registry.dispatch(None, [b"XARITY"]) == b"-ERR wrong number of arguments for 'xarity' command\r\n"
        assert registry.dispatch(None, [b"XARITY", b"a"]) == b"+XOK\r\n"
        assert registry.dispatch(None, [b"XARITY", b"a", b"b", b"c"]) == b"+XOK\r\n"
...
def test_a_duplicate_registration_is_refused_rather_than_shadowing():
    ...
    with pytest.raises(ValueError) as exc_info:
        registry.command(b"PING", arity=-1, kind=registry.Kind.OTHER)(lambda conn, argv: b"+WRONG\r\n")
    assert b"PING" in str(exc_info.value).encode()
    assert commands.dispatch(None, [b"PING"]) == b"+PONG\r\n"


def test_every_handler_returns_bytes_and_writes_nothing_to_the_socket():
    ...
    for argv in ([b"PING"], [b"PING", b"x"], [b"ECHO", b"x"], [b"HELLO"], [b"HELLO", b"9"], [b"NOSUCHCMD"]):
        assert type(commands.dispatch(c, argv)) is bytes, argv
